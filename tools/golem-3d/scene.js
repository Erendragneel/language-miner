...
export async function create(canvas,outfit={},pickaxeId='standard'){
 ...
 const renderer=new T.WebGLRenderer({canvas,alpha:true,antialias:true,powerPreference:'low-power'});renderer.setPixelRatio(Math.min(devicePixelRatio||1,1.5));renderer.setSize(795,697,false);renderer.outputColorSpace=T.SRGBColorSpace;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.18;renderer.shadowMap.enabled=true;renderer.shadowMap.type=T.PCFSoftShadowMap;
 ...
 scene.add(new T.HemisphereLight('#bde6ff','#233743',2.2));const key=new T.DirectionalLight('#fff0cd',3.0);key.position.set(-3,5,4);key.castShadow=true;key.shadow.mapSize.set(1024,1024);key.shadow.camera.left=-2;key.shadow.camera.right=2;key.shadow.camera.top=3;key.shadow.camera.bottom=-2;key.shadow.normalBias=.015;scene.add(key);const rim=new T.DirectionalLight('#7fdbf7',2.0);rim.position.set(2,3,-3);scene.add(rim);
 const model=gltf.scene;model.position.x=-.65;model.rotation.y=.30;scene.add(model);
 const bones={},rest={},materials=new Set();model.traverse(o=>{if(o.isBone){bones[o.name]=o;rest[o.name]={q:o.quaternion.clone(),p:o.position.clone()};}if(o.isMesh){o.castShadow=true;o.receiveShadow=true;o.frustumCulled=false;for(const m of (Array.isArray(o.material)?o.material:[o.material]))materials.add(m);}});
 const hairStyle={buzz:'Buzzed',undercut:'Buzzed',long:'Long',bob:'Long',ponytail:'Long',twintails:'Buns',bun:'Buns'}[outfit.hairStyle]||'SimpleParted';model.traverse(o=>{if(o.name.startsWith('Hair_'))o.visible=o.name==='Hair_'+hairStyle;});
 for(const m of materials){const c=m.name==='Skin'?colors.skin[outfit.skin]||colors.skin.warm:m.name==='Hair'?colors.hair[outfit.hairColor]||colors.hair.brown:m.name==='Jacket'?colors.top[outfit.shirt||outfit.top]||colors.top.miner:m.name==='Trousers'?colors.pants[outfit.pants]||colors.pants.denim:m.name==='Gloves'?({miner:'#a57542',crystal:'#77dfec'}[outfit.gloves]||'#292b2a'):null;if(c)m.color.set(c);}
 ...
 // Spiky/cropped and swept styles share the base scalp, with sculpted tufts.
 if(['spiky','flamespikes','texturedcrop'].includes(outfit.hairStyle)){const g=new T.Group(),color=colors.hair[outfit.hairColor]||colors.hair.brown;for(let i=0;i<7;i++){const tuft=mesh(new T.ConeGeometry(.038,.095+(i%3)*.02,5),color);tuft.position.set((i-3)*.027,.014+Math.sin(i)*.018,.005+(i%2)*.03);tuft.rotation.z=(3-i)*.12;tuft.rotation.x=.3;g.add(tuft);}bindDetail(g,'Head',V(0,1.816,-.017));}
