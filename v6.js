// Language Miner v6.0 — The Polish Update
(()=>{
'use strict';
...
const TOUR=[
 ...
 {section:'Settings and safety',icon:'⚙️',title:'Settings, installation, saves, and recovery',text:'Use Settings for reading support, voice, Smart Review, text, motion, contrast, audio, explanations, and local safety snapshots.',tips:['Install App or Add to Home Screen is available on the sign-in page and again inside Settings when supported by the device.','Safety snapshots provide local recovery points; restoring one replaces the current saved state.','Use the visible Back buttons to return to the parent menu, and avoid clearing browser storage before exporting a backup.']}
];
function ensureV6(){
 state.v6=Object.assign({tourComplete:false,onboardingGuideOffered:false,textSize:'normal',reducedMotion:false,highContrast:false,colorAssist:false,showFloatingCompanion:true,sfxVolume:70,musicVolume:35,musicEnabled:false,musicTrack:'crystal-garden',explanations:true,portraitLockEnabled:true,feedback:[],storySeen:[],lastSnapshot:0,supporterTier:0},state.v6||{});
...
coachShell=function(){
 ...
 if(!dock){
  dock=document.createElement('div');dock.id='v6CoachDock';dock.className='v6-coach-dock';dock.innerHTML=`<button id="v6CoachButton" class="v6-coach-character" type="button" aria-label="Open Kōji’s study reminders" aria-expanded="false" aria-controls="v6CoachCard" title="Kōji the Mine Gnome — open today’s study plan"><span id="v6CoachBadge" class="v6-coach-badge" hidden></span><span class="v6-gnome" aria-hidden="true"><i class="gnome-hat"></i><i class="gnome-face"></i><i class="gnome-nose"></i><i class="gnome-beard"></i><i class="gnome-body"></i></span><strong>Kōji</strong><small>Study guide</small></button><section id="v6CoachCard" class="v6-coach-popover" aria-hidden="true" aria-label="Kōji’s study reminders"><header><div class="v6-coach-heading"><span>KŌJI · MINE GNOME</span><h2>Today’s plan</h2></div><div class="v6-coach-header-actions"><a class="v6-coach-social v6-coach-discord" href="https://discord.gg/AGrGMFjXQm" target="_blank" rel="noopener noreferrer" aria-label="Join the Language Miner Discord" title="Join Discord"><span aria-hidden="true">D</span><strong>Discord</strong></a><a class="v6-coach-social v6-coach-patreon" href="https://www.patreon.com/cw/Erendragneel/membership" target="_blank" rel="noopener noreferrer" aria-label="Support Language Miner on Patreon" title="Open Patreon"><span aria-hidden="true">P</span><strong>Patreon</strong></a><button id="v6CoachClose" type="button" aria-label="Close Kōji’s reminders">×</button></div></header><div id="v6CoachContent"></div></section>`;
...
function applySettings(){ensureV6();window.LanguageMinerMusic?.sync();document.documentElement.dataset.textSize=state.v6.textSize;document.body.dataset.reducedMotion=String(!!state.v6.reducedMotion);document.body.dataset.highContrast=String(!!state.v6.highContrast);document.body.dataset.colorAssist=String(!!state.v6.colorAssist);syncCompanionDisplayButton();}
function openSettings(){
 ensureV6();
 openOverlay('settings');
 const box=document.getElementById('v6SettingsContent');
 box.innerHTML=`<div class="v6-settings-grid"><label>Text size<select id="v6TextSize"><option value="normal">Normal</option><option value="large">Large</option><option value="xlarge">Extra large</option></select></label><label class="v6-switch"><input id="v6ReducedMotion" type="checkbox"><span>Reduce animations</span></label><label class="v6-switch"><input id="v6HighContrast" type="checkbox"><span>High contrast</span></label><label class="v6-switch"><input id="v6ColorAssist" type="checkbox"><span>Color-blind indicators</span></label><label class="v6-switch"><input id="v6Explanations" type="checkbox"><span>Answer explanations</span></label><label class="v6-switch"><input id="v6FloatingCompanion" type="checkbox"><span>Show equipped pet beside Menu</span></label><label>Sound effects <output id="v6SfxOut"></output><input id="v6Sfx" type="range" min="0" max="100"></label><div id="v6MusicPanel"></div></div><section class="v6-save-recovery"><h3>🛟 Save recovery</h3><p>Language Miner keeps rolling safety snapshots on this device.</p><div id="v6SnapshotList"></div><button id="v6SnapshotNow">Create safety snapshot</button></section><section class="v6-save-management"><h3>🗑️ Save management</h3><p>Reset only the currently signed-in player’s progress. You will be asked to confirm before anything is erased.</p><button id="v6ResetSave" class="danger" type="button">Reset Save</button></section>`;
 ...
 box.querySelector('#v6InstallInstructions')?.insertAdjacentHTML('afterend',`<section class="v6-learning-preferences"><div class="v6-learning-preferences-head"><span>LEARNING &amp; AUDIO</span><h3>🔊 Question support</h3><p>Reading, answer audio, native voice, and Smart Review controls now live here.</p></div><div class="v6-settings-grid"><label>Reading support<select id="v6SupportMode"><option value="guided">Guided — readings and furigana</option><option value="standard">Standard — normal furigana</option><option value="challenge">Challenge — reduced furigana</option></select></label><label class="v6-switch"><input id="v6AnswerSounds" type="checkbox"><span>Answer sounds</span></label><label class="v6-switch"><input id="v6VoiceEnabled" type="checkbox"><span>Learning-language voice</span></label><label class="v6-switch"><input id="v6AutoSpeak" type="checkbox"><span>Speak new questions automatically</span></label><label class="v6-switch"><input id="v6SmartReview" type="checkbox"><span>Smart Review weak questions</span></label><label>Voice speed <output id="v6VoiceRateOut"></output><input id="v6VoiceRate" type="range" min="0.55" max="1.15" step="0.05"></label></div><div class="v6-voice-choice"><strong>Native speakers</strong><p id="v6NativeVoiceNames"></p><div role="group" aria-label="Voice gender"><button type="button" data-v6-voice-gender="male">👨 Male</button><button type="button" data-v6-voice-gender="female">👩 Female</button></div><strong>Style</strong><div role="group" aria-label="Voice style"><button type="button" data-v6-voice-style="natural">🗣️ Natural</button><button type="button" data-v6-voice-style="deep">🐢 Deliberate</button><button type="button" data-v6-voice-style="high">☀️ Bright</button><button type="button" data-v6-voice-style="soft">🌙 Soft</button><button type="button" data-v6-voice-style="energetic">⚡ Energetic</button><button type="button" data-v6-voice-style="calm">🍃 Calm</button></div><button id="v6TestVoice" type="button">🔊 Test learning-language voice</button></div></section>`);
 ...
 [['v6AnswerSounds','soundEnabled'],['v6VoiceEnabled','voiceEnabled'],['v6AutoSpeak','autoSpeak'],['v6SmartReview','smartReview']].forEach(([id,key])=>{const control=document.getElementById(id);control.checked=state[key]!==false;control.onchange=()=>{state[key]=control.checked;save();if(key==='voiceEnabled'&&!control.checked)window.LanguageMinerSpeech?.cancel();if(key==='soundEnabled'&&control.checked)playFeedbackSound(true);};});
 const syncVoiceChoices=()=>{const lang=window.LanguageMinerCourseVoice?.currentLanguage?.()||'ja',pack=window.LanguageMinerPronunciation?.pack(lang);const label=box.querySelector('#v6NativeVoiceNames');if(label)label.textContent=pack?`${pack.label}: ${pack.voices.map(v=>v.name).join(' / ')} · Neural voices · Included with the game`:'Using this device’s language voices';box.querySelectorAll('[data-v6-voice-gender]').forEach(button=>{const selected=button.dataset.v6VoiceGender===state.voiceGender;button.classList.toggle('primary',selected);button.setAttribute('aria-pressed',String(selected));const v=pack?.voices.find(v=>v.gender===button.dataset.v6VoiceGender);if(v)button.textContent=v.name+' · '+(v.gender==='male'?'Male':'Female');});box.querySelectorAll('[data-v6-voice-style]').forEach(button=>button.classList.toggle('primary',button.dataset.v6VoiceStyle===state.voiceStyle));};syncVoiceChoices();
 ...
 [['v6Sfx','sfxVolume','v6SfxOut']].forEach(([id,key,out])=>{const e=document.getElementById(id),o=document.getElementById(out);e.value=state.v6[key];o.textContent=e.value+'%';e.oninput=()=>{state.v6[key]=Number(e.value);o.textContent=e.value+'%';save();};});
 window.LanguageMinerMusic?.mount(document.getElementById('v6MusicPanel'));
...
window.LanguageMinerMusic?.configure({read:()=>state.v6||{},write:patch=>{ensureV6();Object.assign(state.v6,patch);save();},active:()=>!!window.japaneseMinerActiveProfile?.()});
init();
})();
