...
(()=>{
  ...
  let selectedLearnerId='',activeTab='overview',activeView='dashboard',studentEmailDraft='';
  ...
  async function syncCloudData(announce=false){
    if(!announce&&centerOpen()&&activeView==='link')return false;
    ...
    if(cloudBusy)return false;cloudBusy=true;cloudError='';
    try{
  ...
  function startCloudRefresh(){clearInterval(cloudTimer);cloudTimer=setInterval(()=>{if(cloudReady()&&(!document.hidden||phoneAlertsEnabled()))syncCloudData(false);},30000);}
  ...
  function openCenter(view='dashboard'){
    ...
    seedPreviewAccess();ensureShell();activeView=['dashboard','manage','link','self'].includes(view)?view:'dashboard';
  ...
  function refreshHeader(){const root=document.getElementById('parentTeacherCenter');if(!root)return;root.querySelector('.ptc-head>div>span').textContent=t(activeView==='self'?'YOUR LEARNING JOURNEY':'READ-ONLY LEARNER INSIGHTS');document.getElementById('ptcTitle').textContent=activeView==='self'?`📊 ${t('My Progress & Grades')}`:`🏫 ${t('Parent/Teacher Center')}`;document.getElementById('ptcBackMenu').textContent=`← ${t('Menu')}`;document.getElementById('ptcClose').setAttribute('aria-label',t(activeView==='self'?'Close My Progress & Grades':'Close Parent/Teacher Center'));}
  ...
  function summaryHeader(summary){return `<section class="ptc-learner-head"><div class="ptc-learner-avatar">${esc(summary.profile.name.slice(0,1).toUpperCase())}</div><div><span>${esc(t('LEARNER REPORT'))}</span><h3>${esc(summary.profile.name)}</h3><p>${esc(languageName(summary.course.known,summary.course.knownName))} → ${esc(languageName(summary.course.learning,summary.course.learningName))} · ${esc(t('Player Level'))} ${summary.level}${summary.cloudUpdatedAt?` · ${esc(t('Synced'))} ${esc(cloudTime(summary.cloudUpdatedAt))}`:''}</p></div><div class="ptc-report-actions"><button type="button" data-ptc-export>⬇ ${esc(t('CSV'))}</button><button type="button" data-ptc-print>🖨 ${esc(t('Print'))}</button><b>🔒 ${esc(t('Read-only'))}</b></div></section>`;}
  function summaryMetrics(summary){return `<section class="ptc-metrics">${metric('Suggested grade',summary.grading.available?`${summary.grading.letter} · ${summary.grading.score}%`:'—','Weighted from available learning evidence','📋')}${metric('Assessment average',summary.assessments.gradedCount?`${summary.assessments.averageScore}%`:'—',`${summary.assessments.attemptCount} recorded attempt${summary.assessments.attemptCount===1?'':'s'}`,'📝')}${metric('Lessons complete',`${summary.course.lessons.counts.completed}/${summary.course.lessons.counts.total}`,`${summary.course.lessons.completionPercent}% of lessons complete`,'📚')}${metric('Current streak',`${summary.activity.currentStreak} ${t(summary.activity.currentStreak===1?'day':'days')}`,summary.activity.streakMaintained?'Maintained today':summary.activity.streakAtRisk?'At risk today':'Current calendar streak','🔥')}</section>`;}
  function tabs(){return `<nav class="ptc-tabs" aria-label="${esc(t('Learner report sections'))}">${[['overview','Grade Summary'],['activity','Daily Activity & Streaks'],['course','Lessons & Completion'],['assessments','Quizzes & Tests'],...(activeView==='self'?[['practice','Practice & Rewards']]:[])].map(([id,label])=>`<button type="button" data-ptc-tab="${id}" class="${activeTab===id?'active':''}">${esc(t(label))}</button>`).join('')}</nav>`;}
  function overview(summary){
    const distribution=summary.questions.distribution,max=Math.max(1,...Object.values(distribution)),grading=summary.grading;
    return `<section class="ptc-report-grid"><article class="ptc-report-card ptc-grade-card"><span>${esc(t('GRADE SNAPSHOT'))}</span><div class="ptc-grade-head"><strong>${esc(grading.letter)}</strong><div><h3>${grading.available?`${grading.score}%`:esc(t('Not enough data'))}</h3><p>${esc(t('Suggested progress grade'))}</p></div></div><div class="ptc-grade-components">${grading.components.map(component=>`<div><label><span>${esc(t(component.label))}</span><b>${component.score}% · ${component.weight}% weight</b></label><i><b style="width:${component.score}%"></b></i></div>`).join('')||`<p>${esc(t('More learning evidence is needed before a grade can be calculated.'))}</p>`}</div><small class="ptc-grade-formula">${esc(grading.formula)}</small></article><article class="ptc-report-card"><span>${esc(t('LEARNING CHECK'))}</span><h3>${esc(t('What needs attention'))}</h3><div class="ptc-learning-flags">${grading.flags.map(flag=>`<p class="${esc(flag.level)}"><span>${flag.level==='good'?'✓':flag.level==='attention'?'!':'•'}</span>${esc(t(flag.text))}</p>`).join('')}</div></article><article class="ptc-report-card"><span>${esc(t('LEARNING EVIDENCE'))}</span><h3>${esc(t('Progress at a glance'))}</h3><div class="ptc-facts"><p><small>${esc(t('Questions answered'))}</small><strong>${summary.questions.answered.toLocaleString()}</strong></p><p><small>${esc(t('Practice accuracy'))}</small><strong>${summary.questions.accuracy}%</strong></p><p><small>${esc(t('Lessons completed'))}</small><strong>${summary.course.lessons.counts.completed}/${summary.course.lessons.counts.total}</strong></p><p><small>${esc(t('Quiz/test average'))}</small><strong>${summary.assessments.gradedCount?`${summary.assessments.averageScore}%`:'—'}</strong></p><p><small>${esc(t('Total study time'))}</small><strong>${duration(summary.activity.totalMilliseconds)}</strong></p><p><small>${esc(t('Active days (30)'))}</small><strong>${summary.activity.activeDaysLast30}</strong></p></div></article><article class="ptc-report-card"><span>${esc(t('PRACTICE DISTRIBUTION'))}</span><h3>${esc(t('What has been practiced'))}</h3><div class="ptc-bars">${Object.entries(distribution).map(([name,value])=>`<div><label><span>${esc(t(name[0].toUpperCase()+name.slice(1)))}</span><b>${value.toLocaleString()}</b></label><i><b style="width:${Math.round(value/max*100)}%"></b></i></div>`).join('')}</div></article><article class="ptc-report-card ptc-wide"><span>${esc(t('UPCOMING STUDY'))}</span><h3>${esc(t('Review status'))}</h3><div class="ptc-review-facts"><p><strong>${summary.reviews.dueCount}</strong><span>${esc(t('due now'))}</span></p><p><strong>${summary.reviews.scheduledCount}</strong><span>${esc(t('scheduled items'))}</span></p><p><strong>${summary.reviews.completedReviews}</strong><span>${esc(t('completed reviews'))}</span></p><p><strong>${summary.reviews.nextDueAt?dateLabel(summary.reviews.nextDueAt):t('None scheduled')}</strong><span>${esc(t('next future review'))}</span></p></div></article></section>`;
  }
  function recentCalendar(summary){
    ...
    let run=0,previous='';const streakByDate=new Map(summary.activity.days.map(day=>{const current=new Date(`${day.date}T12:00:00`),prior=previous?new Date(`${previous}T12:00:00`):null;run=prior&&Math.round((current-prior)/86400000)===1?run+1:1;previous=day.date;return [day.date,run];})),recent=summary.activity.recentDays;
    const first=new Date(today.getFullYear(),today.getMonth(),today.getDate()-34),weekdays=Array.from({length:7},(_,index)=>new Date(first.getFullYear(),first.getMonth(),first.getDate()+index).toLocaleDateString(locale(),{weekday:'short'}));
    return `<section class="ptc-report-grid"><article class="ptc-report-card ptc-wide"><span>${esc(t('ATTENDANCE & TIME'))}</span><h3>${esc(t('Daily learning activity'))}</h3><div class="ptc-review-facts"><p><strong>${summary.activity.activeDaysLast7}/7</strong><span>${esc(t('active days this week'))}</span></p><p><strong>${summary.activity.activeDaysLast30}/30</strong><span>${esc(t('active days this month'))}</span></p><p><strong>${duration(summary.activity.averageDailyMilliseconds)}</strong><span>${esc(t('average timed day'))}</span></p><p><strong>${summary.activity.streakMaintained?'Maintained':summary.activity.streakAtRisk?'At risk':summary.activity.currentStreak?'Current':'No streak'}</strong><span>${summary.activity.currentStreak} ${esc(t('day streak'))}</span></p></div></article><article class="ptc-report-card ptc-wide"><span>${esc(t('LAST 35 DAYS'))}</span><h3>${esc(t('Activity calendar'))}</h3><div class="ptc-calendar-week">${weekdays.map(day=>`<b>${esc(day)}</b>`).join('')}</div><div class="ptc-calendar">${cells.join('')}</div></article><article class="ptc-report-card ptc-wide"><span>${esc(t('DAILY LOG'))}</span><h3>${esc(t('Recorded active study time and streaks'))}</h3>${recent.length?`<div class="ptc-table"><div class="ptc-table-head"><span>${esc(t('Date'))}</span><span>${esc(t('Active study time'))}</span><span>${esc(t('Streak after study'))}</span></div>${recent.map(day=>`<div><strong>${esc(dateLabel(day.date))}</strong><span>${esc(day.milliseconds?duration(day.milliseconds):t('Activity recorded'))}</span><b>🔥 ${streakByDate.get(day.date)||1} ${esc(t((streakByDate.get(day.date)||1)===1?'day':'days'))}</b></div>`).join('')}</div>`:`<div class="ptc-inline-empty">${esc(t('No study days have been recorded yet.'))}</div>`}</article></section>`;
  }
  function courseReport(summary){const report=summary.course.lessons,groups=new Map();report.items.forEach(lesson=>{const key=`${lesson.level} · ${lesson.section}`,items=groups.get(key)||[];items.push(lesson);groups.set(key,items);});return `<section class="ptc-report-grid"><article class="ptc-report-card ptc-wide"><span>${esc(t('LESSON COMPLETION'))}</span><h3>${esc(languageName(summary.course.learning,summary.course.learningName))} ${esc(t('course gradebook'))}</h3><div class="ptc-review-facts"><p><strong>${report.counts.completed}</strong><span>${esc(t('completed lessons'))}</span></p><p><strong>${report.counts.inProgress}</strong><span>${esc(t('in progress'))}</span></p><p><strong>${report.counts.available}</strong><span>${esc(t('not started'))}</span></p><p><strong>${report.counts.locked}</strong><span>${esc(t('locked lessons'))}</span></p></div><div class="ptc-course-overall"><strong>${report.completionPercent}%</strong><i><b style="width:${report.completionPercent}%"></b></i><small>${report.counts.completed}/${report.counts.total} ${esc(t('lessons complete'))} · ${report.averageMastery}% ${esc(t('average mastery'))}</small></div></article><article class="ptc-report-card ptc-wide"><span>${esc(t('LEVEL PROGRESS'))}</span><h3>${esc(t('Course levels'))}</h3><div class="ptc-levels">${summary.course.levels.map(level=>{const xpPercent=Math.min(100,Math.round(level.xp/Math.max(1,level.xpRequired)*100));return `<article class="${level.selected?'selected':''} ${level.completed?'completed':''} ${!level.unlocked?'locked':''}"><span>${level.completed?'✓':level.unlocked?'◆':'🔒'}</span><div><strong>${esc(courseLevelName(summary.course,level))}</strong><small>${level.masteryRequired?`${level.mastery}% ${esc(t('mastery'))} · `:''}${level.xp.toLocaleString()} / ${level.xpRequired.toLocaleString()} XP</small><i><b style="width:${level.completed?100:xpPercent}%"></b></i></div><em>${esc(t(level.completed?'Complete':level.selected?'Current':level.unlocked?'Available':'Locked'))}</em></article>`;}).join('')}</div></article>${[...groups].map(([label,lessons])=>`<article class="ptc-report-card ptc-wide ptc-lesson-group"><span>${esc(label.toUpperCase())}</span><h3>${esc(label)}</h3><div class="ptc-table ptc-lesson-table"><div class="ptc-table-head"><span>${esc(t('Lesson'))}</span><span>${esc(t('Completion'))}</span><span>${esc(t('Status'))}</span></div>${lessons.map(lesson=>`<div><strong>${esc(lesson.name)}<small>${lesson.masteredItems}/${lesson.totalItems} ${esc(t('items at target'))}</small></strong><span><b>${lesson.mastery}%</b><i><b style="width:${lesson.mastery}%"></b></i></span><em class="${lesson.completed?'complete':lesson.unlocked?'active':'locked'}">${lesson.completed?'✓ Complete':lesson.unlocked?(lesson.mastery?'In progress':'Not started'):'🔒 Locked'}</em></div>`).join('')}</div></article>`).join('')}</section>`;}
  function assessments(summary){
    const fastest=summary.assessments.fastest,history=summary.assessments.history;
    const modeLabel=mode=>mode==='easy'?'🌱 Easy':mode==='hard'?'⛏️ Hard':mode==='placement'?'🧭 Placement':'— Not recorded';
    return `<section class="ptc-report-grid"><article class="ptc-report-card ptc-wide"><span>${esc(t('ASSESSMENT SUMMARY'))}</span><h3>${esc(t('Quiz and test performance'))}</h3><p>Placement tests and unfinished assessments are retained in history but excluded from the grade and average.</p><div class="ptc-review-facts"><p><strong>${summary.assessments.gradedCount?`${summary.assessments.averageScore}%`:'—'}</strong><span>${esc(t('average score'))}</span></p><p><strong>${summary.assessments.attemptCount}</strong><span>${esc(t('total attempts'))}</span></p><p><strong>🌱 ${summary.assessments.easyCount}</strong><span>${esc(t('Easy attempts'))}</span></p><p><strong>⛏️ ${summary.assessments.hardCount}</strong><span>${esc(t('Hard attempts'))}</span></p></div>${summary.assessments.modeUnknownCount?`<p class="ptc-history-note">${summary.assessments.modeUnknownCount} older summary record${summary.assessments.modeUnknownCount===1?' does':'s do'} not contain a saved Easy/Hard mode. New attempts record it automatically.</p>`:''}</article><article class="ptc-report-card ptc-wide"><span>${esc(t('PERSONAL BESTS'))}</span><h3>${esc(t('Fastest successful completions'))}</h3><div class="ptc-records">${[['placement','Placement test'],['reviewQuiz','Passing review quiz'],['guardian','Perfect Guardian test'],['practiceTest','Practice test']].map(([key,label])=>`<div class="${fastest[key]?'has-record':''}"><span>${esc(t(label))}</span><strong>${esc(recordTime(fastest[key]))}</strong><small>${esc(t(fastest[key]?'Fastest eligible completion':'No eligible result yet'))}</small></div>`).join('')}</div></article><article class="ptc-report-card ptc-wide"><span>${esc(t('ATTEMPT HISTORY'))}</span><h3>${esc(t('Every recorded quiz and test result'))}</h3>${history.length?`<div class="ptc-table ptc-assessment-table"><div class="ptc-table-head"><span>${esc(t('Assessment'))}</span><span>${esc(t('Score & result'))}</span><span>${esc(t('Mode'))}</span><span>${esc(t('Date & time'))}</span></div>${history.map(record=>`<div><strong>${esc(recordLabel(record.type))}<small>${esc(record.course)}${record.section?` · ${esc(record.section)}`:''}${record.lessons?` · Lessons ${esc(record.lessons)}`:''}</small></strong><span><b>${record.score}%</b> · ${esc(t(record.result))}<small>${record.total?`${record.correct}/${record.total} correct · ${record.answered}/${record.total} answered`:`${record.attempts} saved attempt${record.attempts===1?'':'s'}`}</small></span><em class="ptc-mode ${esc(record.difficulty)}">${esc(modeLabel(record.difficulty))}</em><span>${record.completedAt?esc(dateLabel(record.completedAt)):esc(t('Historical summary'))}<small>${record.durationMs||record.fastestTimeMs?`${esc(recordTime(record.durationMs||record.fastestTimeMs))} elapsed`:record.legacy?esc(t('Exact date not recorded')):''}</small></span></div>`).join('')}</div>`:`<div class="ptc-inline-empty">${esc(t('No assessment attempts have been recorded yet.'))}</div>`}</article></section>`;
  }
  function selectedSummary(){if(activeView==='self')return bridge()?.learnerSummary?.(current()?.id);return selectedLearnerId?(selectedLearnerId.startsWith('cloud:')?cloudSummaries.get(selectedLearnerId):bridge()?.learnerSummary?.(selectedLearnerId)):null;}
  function csvCell(value){let text=String(value??'');if(/^[=+\-@]/.test(text))text=`'${text}`;return /[",\r\n]/.test(text)?`"${text.replace(/"/g,'""')}"`:text;}
  function exportSummary(summary){
    if(!summary)return;const rows=[['Language Miner Learner Report'],['Student',summary.profile.name],['Course',summary.course.learningName],['Generated',new Date(summary.generatedAt).toLocaleString()],[],['Grade Summary'],['Suggested grade',summary.grading.letter],['Suggested score',summary.grading.available?`${summary.grading.score}%`:'Not graded'],['Assessment average',summary.assessments.gradedCount?`${summary.assessments.averageScore}%`:'Not graded'],['Completed-lesson practice accuracy',summary.questions.answered?`${summary.questions.accuracy}%`:'Not graded'],['Lessons completed',`${summary.course.lessons.counts.completed}/${summary.course.lessons.counts.total}`],['Completed lesson mastery',summary.course.lessons.counts.completed?`${summary.course.lessons.completedAverageMastery}%`:'Not graded'],['Total active study time',duration(summary.activity.totalMilliseconds)],['Current streak',`${summary.activity.currentStreak} days`],['Best streak',`${summary.activity.bestStreak} days`],[],['Daily Activity'],['Date','Active study milliseconds','Active study time']];summary.activity.days.forEach(day=>rows.push([day.date,day.milliseconds,duration(day.milliseconds)]));rows.push([],['Lesson Progress'],['Course level','Section','Lesson','Mastery','Mastered items','Total items','Status']);summary.course.lessons.items.forEach(lesson=>rows.push([lesson.level,lesson.section,lesson.name,`${lesson.mastery}%`,lesson.masteredItems,lesson.totalItems,lesson.completed?'Complete':lesson.unlocked?(lesson.mastery?'In progress':'Not started'):'Locked']));rows.push([],['Quiz and Test Attempts'],['Date','Assessment','Course','Level','Section','Lessons','Mode','Score','Correct','Total','Answered','Result','Elapsed milliseconds']);summary.assessments.history.forEach(record=>rows.push([record.completedAt?new Date(record.completedAt).toISOString():'',record.type,record.course,record.level,record.section,record.lessons,record.difficulty,`${record.score}%`,record.correct,record.total,record.answered,record.result,record.durationMs||record.fastestTimeMs||0]));rows.push([],['Learning Alerts'],['Level','Message']);summary.grading.flags.forEach(flag=>rows.push([flag.level,flag.text]));const csv='\uFEFF'+rows.map(row=>row.map(csvCell).join(',')).join('\r\n'),blob=new Blob([csv],{type:'text/csv;charset=utf-8'}),url=URL.createObjectURL(blob),link=document.createElement('a'),safeName=summary.profile.name.replace(/[^a-z0-9_-]+/gi,'-').replace(/^-|-$/g,'')||'learner';link.href=url;link.download=`Language-Miner-${safeName}-report-${new Date().toISOString().slice(0,10)}.csv`;document.body.appendChild(link);link.click();link.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);window.setMessage?.(t('Learner report CSV downloaded.'),'correct');
  }
  function selfReport(){
  ...
  async function handleSubmit(event){
    ...
    if(previewCloudMode()){
      ...
      cloudLastSync=Date.now();lastDeliveryReceipt={id:pending.cloudId,email,status:pending.status,at:Date.now()};studentEmailDraft='';activeView='manage';window.setMessage?.(t('Preview request created. Production requests go to the learner’s signed-in account.'),'correct');render();return;
    }
    cloudBusy=true;cloudError='';render();
      try{
        ...
        cloudLinks=verifiedRows.map(normalizeCloudLink);cloudLastSync=Date.now();lastDeliveryReceipt={id:String(verified.id),email,status:String(verified.status),at:Date.now()};studentEmailDraft='';activeView='manage';
  ...
  async function handleAction(event){
    ...
    if(activeView==='self'&&button.id==='openStatsCalendarBtn'){window.LanguageMinerPersonalStats?.calendar();return;}
    if(activeView==='self'&&button.id==='openStatsPlacementBtn'){window.LanguageMinerPersonalStats?.placement(button.dataset.courseLanguage);return;}
  ...
  function addMenuItem(){
    ...
    const own=document.createElement('button');own.type='button';own.dataset.myProgress='1';own.dataset.menuCategoryName='player';own.innerHTML='<span>📊</span><strong>My Progress & Grades</strong><small>Your grades, lessons, study time, and quiz results</small>';own.onclick=()=>{window.closeGameMenu?.();activeTab='overview';openCenter('self');};grid.appendChild(own);
    const layout=grid.closest('.miner-interface-menu');if(layout){button.hidden=layout.dataset.category!=='player';own.hidden=button.hidden;}window.LanguageMinerI18n?.localize?.(own);window.LanguageMinerI18n?.localize?.(button);window.refreshJapaneseMinerFeatureMenu?.();
  }
  function init(){ensureShell();addMenuItem();syncRequestNotification();startCloudRefresh();if(cloudReady())syncCloudData(false);}
  document.addEventListener('input',event=>{if(event.target.id==='ptcStudentEmail')studentEmailDraft=event.target.value;});
  ...
  window.addEventListener('jm-profile-loaded',()=>{studentEmailDraft='';selectedLearnerId='';activeTab='overview';activeView='dashboard';cloudLinks=[];cloudSummaries=new Map();cloudError='';cloudLastSync=0;lastDeliveryReceipt=null;setTimeout(init,0);});
  window.addEventListener('jm-profile-logged-out',()=>{studentEmailDraft='';closeCenter();clearInterval(cloudTimer);cloudTimer=0;cloudLinks=[];cloudSummaries=new Map();cloudError='';cloudLastSync=0;});
  ...
  window.openLanguageMinerMyProgress=()=>{activeTab='overview';return openCenter('self');};
