...
(()=>{
  ...
  function summaryMetrics(summary){return `<section class="ptc-metrics">${metric('Suggested grade',summary.grading.available?`${summary.grading.letter} · ${summary.grading.score}%`:'—','Weighted from available learning evidence','📋')}${metric('Assessment average',summary.assessments.history.length?`${summary.assessments.averageScore}%`:'—',`${summary.assessments.attemptCount} recorded attempt${summary.assessments.attemptCount===1?'':'s'}`,'📝')}${metric('Lessons complete',`${summary.course.lessons.counts.completed}/${summary.course.lessons.counts.total}`,`${summary.course.lessons.completionPercent}% of lessons complete`,'📚')}${metric('Current streak',`${summary.activity.currentStreak} ${t(summary.activity.currentStreak===1?'day':'days')}`,summary.activity.streakMaintained?'Maintained today':summary.activity.streakAtRisk?'At risk today':'Current calendar streak','🔥')}</section>`;}
