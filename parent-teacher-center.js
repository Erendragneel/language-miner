...
(()=>{
  ...
  function tabs(){return `<nav class="ptc-tabs" aria-label="${esc(t('Learner report sections'))}">${[['overview','Grade Summary'],['activity','Daily Activity & Streaks'],['course','Lessons & Completion'],['assessments','Quizzes & Tests']].map(([id,label])=>`<button type="button" data-ptc-tab="${id}" class="${activeTab===id?'active':''}">${esc(t(label))}</button>`).join('')}</nav>`;}
  ...
  function selectedSummary(){return selectedLearnerId?(selectedLearnerId.startsWith('cloud:')?cloudSummaries.get(selectedLearnerId):bridge()?.learnerSummary?.(selectedLearnerId)):null;}
  ...
  window.addEventListener('jm-profile-logged-out',()=>{studentEmailDraft='';clearInterval(cloudTimer);cloudTimer=0;cloudLinks=[];cloudSummaries=new Map();cloudError='';cloudLastSync=0;});
