...
(()=>{
  ...
  function answerCourseReview(button){
    const quiz=multilingualReviewQuiz;if(!quiz||quiz.finished||quiz.answered)return;if(courseReviewRemainingMs(quiz)<=0){finishCourseReview('timeout');return;}const question=quiz.questions[quiz.index],selected=question.options[Number(button.dataset.lmReviewAnswer)],correct=vocabularyAnswerAccepted(question,selected);quiz.selected=selected;quiz.answered=true;quiz.answeredCount=Number(quiz.answeredCount||0)+1;if(correct)quiz.correct=Number(quiz.correct||0)+1;const progress=languageProgress(),section=recordCourseSectionAnswer(progress,question.sourceSection||quiz.section,correct);saveLanguageProgress(progress);window.LanguageMinerEconomy?.recordCourseAnswer?.({correct,stage:quiz.mine,section,review:true,deferTreasure:true});
