...
(()=>{
  ...
  const ALPHABET_SYSTEMS={
    ...
    zh:{name:'Mandarin Pinyin Alphabet & Tones',summary:'Pinyin initials, finals, and four tones',units:alphabetUnits('b~b (bā)~八|p~p (pā)~趴|m~m (mā)~妈|f~f (fā)~发|d~d (dā)~搭|t~t (tā)~他|n~n (nā)~拿|l~l (lā)~拉|g~g (gē)~哥|k~k (kē)~科|h~h (hē)~喝|j~j (jī)~鸡|q~q (qī)~七|x~x (xī)~西|zh~zh (zhī)~知|ch~ch (chī)~吃|sh~sh (shī)~师|r~r (rì)~日|z~z (zī)~资|c~c (cī)~疵|s~s (sī)~思|y~y (yī)~一|w~w (wū)~屋|a~final a~啊|o~final o~喔|e~final e~鹅|i~final i~衣|u~final u~乌|ü~final ü~迂|ā~first tone~妈|á~second tone~麻|ǎ~third tone~马|à~fourth tone~骂')},
  ...
  function saveSettings(next,allowPlacementReset=false){if(!allowPlacementReset){const previous=readSettings();next.placements=next.placements||{};Object.entries(previous.placements||{}).forEach(([id,record])=>{if(placementRecordComplete(record))next.placements[id]=record;});}saveSettingsFor(accountKey(),next);window.dispatchEvent(new CustomEvent('lm-course-settings-saved'));}
  ...
  function placementMineTitle(languageId,stage){return languageId==='ja'?(EXPEDITION_COURSES.ja[Number(stage)]?.[0]||'Japanese'): `${targetName(languageId)} · ${placementLevelLabel(languageId,stage)}`;}
  ...
  function placementCourseStageQuestions(targetId,knownId,stage,count){
    ...
    ['vocabulary','grammar','sentences'].forEach(section=>{const levelItems=courseSectionLessonsFor(targetId,section,stage).flat().filter(item=>item?.forms?.[targetId]&&item?.forms?.[knownId]),fallback=(multilingualCourseData()[section]||[]).filter(item=>window.LanguageMinerVocabulary?.available(item)!==false&&item?.forms?.[targetId]&&item?.forms?.[knownId]);pools[section]=shuffled(levelItems.length?levelItems:fallback);offsets[section]=0;});
  ...
  function answerMultilingualPlacement(button,skipped=false){
    ...
    const question=session.questions[session.index],selected=button?question.options[Number(button.dataset.lmPlacementOption)]:'',correct=!skipped&&vocabularyAnswerAccepted(question,selected,session.learning,session.known);session.answered=true;if(correct)session.correct++;if(skipped)session.skipped++;session.answers.push({stage:question.stage,section:question.section,correct,skipped});
    ...
    content.querySelectorAll('[data-lm-placement-option]').forEach(option=>{option.disabled=true;const value=question.options[Number(option.dataset.lmPlacementOption)];if(vocabularyAnswerAccepted(question,value,session.learning,session.known))option.classList.add('correct');else if(option===button)option.classList.add('wrong');});
  ...
  function finishMultilingualPlacement(){
    if(multilingualPlacement&&placementRecordComplete(currentSettings().placements[multilingualPlacement.learning])){multilingualPlacement=null;renderPlacement();return;}
  ...
  function skipPlacement(){if(placementRecordComplete(currentSettings().placements[learning])||originalJapanesePlacementComplete()){renderPlacement();return;}persistPair('beginner','full');hideOriginalJapanesePlacementForOtherCourses();if(learning==='ja'){closeFlow();if(window.openPlacementOnboarding?.(true))window.chooseBrandNew?.();return;}openPostPlacementGuide();}
  function startPlacement(){if(placementRecordComplete(currentSettings().placements[learning])||originalJapanesePlacementComplete()){renderPlacement();return;}persistPair(null,'full');if(learning==='ja'){closeFlow();if(window.openPlacementOnboarding?.(true))window.startPlacementTest?.();return;}hideOriginalJapanesePlacementForOtherCourses();beginMultilingualPlacement();}
  ...
  function vocabularyAnswerAccepted(question,selected,targetId=learning,knownId=known){return window.LanguageMinerVocabulary?.multilingualAccepts(question,selected,targetId,knownId)||selected===question.answer;}
  ...
  function courseSectionLessonsFor(languageId,section,mineIndex=selectedCourseMine){
    ...
    for(let offset=0;offset<stageItems.length;offset+=size)result.push(stageItems.slice(offset,offset+size).filter(item=>window.LanguageMinerVocabulary?.available(item)!==false));
    return result;
  }
  ...
  function recordCourseMastery(question,correct){
    ...
    const section=recordCourseSectionAnswer(progress,question.sourceSection||question.mode,correct),id=courseMasteryId(section,question.item);progress.courseMastery[id]=Math.min(100,Math.max(0,(Number(progress.courseMastery[id])||0)+(correct?25:-5)));progress.itemPracticeStats=progress.itemPracticeStats||{};const attempt=progress.itemPracticeStats[id]||{attempts:0,correct:0};attempt.attempts++;attempt.correct+=correct?1:0;progress.itemPracticeStats[id]=attempt;
    ...
    progress.xp=Number(progress.xp||0)+xpGain;saveLanguageProgress(progress);const economy=window.LanguageMinerEconomy?.recordCourseAnswer?.({correct,learningLanguage:learning,knownLanguage:known,item:question.item,stage:selectedCourseMine,section,review:false,deferTreasure:false})||null;updateFoundationProgress();return economy;
  }
  ...
  function speakLanguage(text,languageId,options={}){
    if(!text)return false;const language=LANGUAGES[languageId]?.voice,engine=window.LanguageMinerSpeech;if(!language)return false;
  ...
  // Shared course controls must never enter the Japanese-only academy for another course.
  function openCurrentCourse(){window.closeGameMenu?.();window.openJapaneseMinerV5?.('map');return true;}
  function selectCurrentCourseMine(index){
    index=Math.max(0,Math.min(travelCourseActive()?0:6,Number(index)||0));
  ...
  window.LanguageMinerCourseCoach=Object.freeze({
    ...
    snapshot:()=>{
      const mine=selectedCourseMine,sections=courseMineSections(mine).filter(section=>section!=='boss');
      let section=sections.includes(selectedCourseSection)?selectedCourseSection:sections[0],lessons=courseSectionLessons(section,mine),lesson=Math.max(0,Math.min(lessons.length-1,selectedCourseLesson));
      const checkpoints=sections.filter(part=>part!=='alphabet').flatMap(part=>courseSectionLessons(part,mine).map((_,index)=>({section:part,evenLesson:index+1}))).find(item=>item.evenLesson%2===0&&courseReviewAvailable(mine,item.section,item.evenLesson)&&!courseReviewPassed(mine,item.section,item.evenLesson));
      if(courseLessonMastery(section,lesson,mine)>=100){const next=sections.flatMap(part=>courseSectionLessons(part,mine).map((_,index)=>({section:part,lesson:index}))).find(item=>courseLessonReplayable(item.section,item.lesson,mine)&&courseLessonMastery(item.section,item.lesson,mine)<100);if(next){section=next.section;lesson=next.lesson;}else lesson=-1;}
      const mastery=lesson>=0?courseLessonMastery(section,lesson,mine):100,requirement=section==='alphabet'?MULTILINGUAL_ALPHABET_BOSS_MASTERY:MULTILINGUAL_LESSON_MASTERY_REQUIREMENT;
      const reminder=checkpoints?{icon:'🧠',title:`${targetName()} · ${sectionLabel(checkpoints.section)} review is ready`,text:`Complete the checkpoint for lessons ${checkpoints.evenLesson-1}–${checkpoints.evenLesson} to continue.`,action:'course-checkpoint',label:'Start review quiz',mine,...checkpoints}:lesson<0?{icon:'✓',title:`${placementMineTitle(learning,mine)} lessons mastered`,text:'These lessons are at 100% mastery. Check your guardian gate or continue to the next unlocked mine.',action:'map',label:'View progression'}:{icon:LANGUAGES[learning].flag,title:`${targetName()} · ${sectionLabel(section)} · Lesson ${lesson+1}`,text:`${mastery}% mastery. ${mastery>=requirement?'Keep practicing toward 100% mastery.':`Reach ${requirement}% mastery, then complete any required checkpoint to continue.`}`,action:'course-lesson',label:'Practice this lesson',mine,section,lesson};
      return {pending:!!activePreviewQuestion||multilingualBoss?.status==='active'||!!multilingualReviewQuiz&&!multilingualReviewQuiz.finished,bossReady:courseBossUnlocked(mine)&&!courseBossDefeated(mine),mineTitle:placementMineTitle(learning,mine),reminder};
    },
    openTask:task=>task.action==='course-checkpoint'?openCourseReview(task.mine,task.section,task.evenLesson):startCourseLesson(task.section,task.lesson,task.mine),
  ...
  function makeMeaningQuestion(section,lesson=selectedCourseLesson){
    ...
    const pictured=window.LanguageMinerPracticeModes?.current()==='picture'?items.filter(item=>window.LanguageMinerPictures?.imageFor(item,{language:learning})):[],practiceItems=pictured.length?pictured:items;
    const optionLimit=4,item=shuffled(practiceItems.filter(candidate=>candidate.id!==previous))[0]||practiceItems[0],answer=item?.forms?.[learning],meaning=item?.forms?.[known],options=[answer];
  ...
  function savedCourseBoss(mineIndex){
    const saved=languageProgress().activeBoss;if(saved?.questions?.some(q=>q.item&&window.LanguageMinerVocabulary?.available(multilingualCourseData()[q.sourceSection]?.find(r=>r.id===q.item.id))===false))return null;if(!saved||Number(saved.mine)!==Number(mineIndex)||!['ready','active','failed','passed'].includes(saved.status))return null;
  ...
  function renderCourseBossQuestion(){
    ...
    window.LanguageMinerVocabulary?.refreshMultilingual(question,learning,known,meaning=>(QUESTION_PROMPTS[known]||QUESTION_PROMPTS.en)(meaning,targetName()));activePreviewQuestion=question;const progress=boss.answeredCount/MULTILINGUAL_BOSS_QUESTION_TARGET*100;
    ...
    document.querySelectorAll('[data-lm-course-answer]').forEach(button=>button.addEventListener('click',()=>answerCourseQuestion(button)));const message=document.getElementById('message');if(message)message.textContent='';const quickLabel=document.getElementById('quickMineLabel');if(quickLabel)quickLabel.textContent='Resume';area.scrollIntoView({behavior:'smooth',block:'center'});startCourseBossClock();
  }
  ...
  function answerCourseBossQuestion(button){
    ...
    if(courseBossRemainingMs(boss)<=0){finishCourseBoss('timeout');return;}
    const selected=button.dataset.lmCourseAnswer,correct=vocabularyAnswerAccepted(question,selected);boss.answered=true;boss.answeredCount=Math.min(MULTILINGUAL_BOSS_QUESTION_TARGET,Number(boss.answeredCount||0)+1);if(correct)boss.correct=Number(boss.correct||0)+1;else boss.missed.push({id:question.id,prompt:question.prompt,answer:question.answer,selected,section:question.sourceSection});const progress=languageProgress(),section=recordCourseSectionAnswer(progress,question.sourceSection||question.mode,correct);saveLanguageProgress(progress);window.LanguageMinerEconomy?.recordCourseAnswer?.({correct,learningLanguage:learning,knownLanguage:known,item:question.item,stage:boss.mine,section,review:false,deferTreasure:true});
    document.querySelectorAll('[data-lm-course-answer]').forEach(item=>{item.disabled=true;if(vocabularyAnswerAccepted(question,item.dataset.lmCourseAnswer))item.classList.add('correct');else if(item===button)item.classList.add('wrong');});const feedback=document.getElementById('lmCourseFeedback');if(feedback){feedback.className=`lm-course-feedback ${correct?'correct':'wrong'}`;feedback.innerHTML=correct?`✓ ${escapeHtml(ui('correct'))}`:escapeHtml(ui('correctAnswerIs',{answer:question.answer}));}persistCourseBoss();
  ...
  function renderCourseReviewQuiz(){
    ...
    const question=quiz.questions[quiz.index],remaining=courseReviewRemainingMs(quiz),progress=(quiz.index+1)/MULTILINGUAL_REVIEW_QUESTION_TARGET*100,record=courseReviewResult(quiz.mine,quiz.section,quiz.evenLesson),recordCopy=record.fastestTimeMs?` · ${ui('recordTime',{time:courseAssessmentTimeLabel(record.fastestTimeMs)})}`:'';window.LanguageMinerVocabulary?.refreshMultilingual(question,learning,known,meaning=>(QUESTION_PROMPTS[known]||QUESTION_PROMPTS.en)(meaning,targetName()));activePreviewQuestion=question;
  ...
  function answerCourseReview(button){
    const quiz=multilingualReviewQuiz;if(!quiz||quiz.finished||quiz.answered)return;if(courseReviewRemainingMs(quiz)<=0){finishCourseReview('timeout');return;}const question=quiz.questions[quiz.index],selected=question.options[Number(button.dataset.lmReviewAnswer)],correct=vocabularyAnswerAccepted(question,selected);quiz.selected=selected;quiz.answered=true;quiz.answeredCount=Number(quiz.answeredCount||0)+1;if(correct)quiz.correct=Number(quiz.correct||0)+1;const progress=languageProgress(),section=recordCourseSectionAnswer(progress,question.sourceSection||quiz.section,correct);saveLanguageProgress(progress);window.LanguageMinerEconomy?.recordCourseAnswer?.({correct,learningLanguage:learning,knownLanguage:known,item:question.item,stage:quiz.mine,section,review:true,deferTreasure:true});
    document.querySelectorAll('[data-lm-review-answer]').forEach((item,index)=>{item.disabled=true;const option=question.options[index];if(vocabularyAnswerAccepted(question,option))item.classList.add('correct');else if(item===button)item.classList.add('wrong');});const feedback=document.getElementById('lmCourseFeedback');if(feedback){feedback.className=`lm-course-feedback ${correct?'correct':'wrong'}`;feedback.innerHTML=correct?`✓ ${escapeHtml(ui('correct'))}`:escapeHtml(ui('correctAnswerIs',{answer:question.answer}));}
  ...
  function renderCourseQuestion(section=selectedCourseSection,lesson=selectedCourseLesson){
    if(section!=='boss'&&window.LanguageMinerPracticeModes?.current()==='writing'){window.LanguageMinerPracticeModes.openWriting();return;}
    ...
    window.LanguageMinerPracticeModes?.apply(area,question,{language:learning,spoken:question.spoken||question.answer,originalPrompt:courseQuestionPrompt(question,target),readingPrompt:alphabetQuestion?(ALPHABET_PROMPTS[known]||ALPHABET_PROMPTS.en)(question.label,targetName()):courseQuestionPrompt(question,target)});
    ...
    syncMultilingualQuickMineButton();updateFoundationProgress();area.scrollIntoView({behavior:'smooth',block:'center'});if(hearingQuestion&&!window.LanguageMinerPracticeModes)setTimeout(()=>{if(activePreviewQuestion===question)speakTarget(question.spoken);},80);
  }
  ...
  function courseLessonGrid(section,mineIndex){
    ...
    return `<div class="lm-mine-lesson-grid lm-${section}-lesson-grid">${lessons.map((items,index)=>{const mastery=courseLessonMastery(section,index,mineIndex),active=selectedCourseMine===mineIndex&&selectedCourseSection===section&&selectedCourseLesson===index;if(section==='boss'){const defeated=courseBossDefeated(mineIndex),unlocked=courseBossUnlocked(mineIndex),fastest=courseAssessmentTime(languageProgress().bossFastestByMine?.[mineIndex]),copy=defeated?`${ui('bossPassedPlan')}${fastest?` · ${ui('recordTime',{time:courseAssessmentTimeLabel(fastest)})}`:''}`:unlocked?ui('bossPlan'):courseBossRequirement(mineIndex);return `<button type="button" class="lm-mine-lesson lm-guardian-lesson ${active?'current':''} ${defeated?'complete':unlocked?'':'locked'}" data-lm-course-mine="${mineIndex}" data-lm-course-section="boss" data-lm-course-lesson="0" ${!unlocked||defeated?'disabled':''}><span><img class="mine-golem-icon" data-boss-mine="${mineIndex}" src="mine-golem-v1.png" alt="" width="48" height="48">${defeated?'✓':''}</span><strong>${escapeHtml(ui('guardianBoss'))}</strong><small>${escapeHtml(copy)}</small><i><b style="width:${defeated?100:Math.min(100,courseMineXp(mineIndex)/MULTILINGUAL_BOSS_XP_REQUIREMENT*100)}%"></b></i><em>${defeated?'25/25':`${mastery}%`}</em></button>`;}const unlocked=courseLessonReplayable(section,index,mineIndex),requirement=section==='alphabet'?MULTILINGUAL_ALPHABET_BOSS_MASTERY:MULTILINGUAL_LESSON_MASTERY_REQUIREMENT,copy=unlocked?courseLessonSubtitle(section,items,index):ui(section==='alphabet'?'alphabetLessonUnlockRequirement':'lessonUnlockRequirement',{mastery:requirement});const card=`<button type="button" class="lm-mine-lesson ${active?'current':''} ${mastery>=requirement?'complete':''} ${unlocked?'':'locked'}" data-lm-course-mine="${mineIndex}" data-lm-course-section="${section}" data-lm-course-lesson="${index}" ${unlocked?'':'disabled'}><span>${unlocked?index+1:'🔒'}</span><strong>${escapeHtml(ui('lesson',{number:index+1}))}</strong><small>${escapeHtml(copy)}</small><i><b style="width:${mastery}%"></b></i><em>${mastery}%</em></button>`;return `${card}${section!=='alphabet'&&(index+1)%2===0?courseReviewCheckpointCard(section,mineIndex,index+1):''}`;}).join('')}</div>`;
  }
  ...
  window.LanguageMinerCourseBosses=Object.freeze({active:()=>!fullJapaneseCourse(),markup:()=>`<div class="lm-course-boss-catalog" data-learning-language="${escapeHtml(learning)}">${Array.from({length:travelCourseActive()?1:7},(_,mine)=>`<section><h3>${escapeHtml(placementMineTitle(learning,mine))}</h3>${courseLessonGrid('boss',mine)}</section>`).join('')}</div>`});
  function courseMineContents(mineIndex){
    ...
    return sections.map(section=>`<section class="lm-stage-course-section"><header class="lm-stage-course-head"><h4>${section==='boss'?'🏯':section==='travel'?'🧳':'📘'} ${escapeHtml(section==='boss'?ui('guardianBoss'):sectionLabel(section))}</h4>${section===modeSection?window.japaneseMinerQuizModeMarkup?.()||'':''}</header>${courseLessonGrid(section,mineIndex)}${['alphabet','vocabulary'].includes(section)?courseSectionLessons(section,mineIndex).map((items,lesson)=>courseLessonReplayable(section,lesson,mineIndex)?`<details class="lm-flashcard-lesson"><summary>${escapeHtml(ui('lesson',{number:lesson+1}))} · ${section==='alphabet'?'click a character':'click a word'}</summary>${window.LanguageMinerFlashcards?.chips(items,{kind:'course',stage:mineIndex,lesson,section,language:learning})||''}</details>`:'').join(''):''}</section>`).join('');
  }
  ...
  function adminResetLanguage(languageId){
    if(!multilingualAdminAllowed()||!LANGUAGES[languageId]||languageId==='ja')return false;const settings=currentSettings();delete settings.progress[languageId];delete settings.placements[languageId];delete settings.guides[languageId];saveSettings(settings,true);refreshAfterAdmin(settings);return true;
  }
  function adminResetAllLanguages(){
    if(!multilingualAdminAllowed())return false;const settings=currentSettings();nonJapaneseLanguageIds().forEach(id=>{delete settings.progress[id];delete settings.placements[id];delete settings.guides[id];});saveSettings(settings,true);refreshAfterAdmin(settings);return true;
  }
  function adminResetBossesAndReviews(){
    if(!multilingualAdminAllowed())return false;const settings=currentSettings();nonJapaneseLanguageIds().forEach(id=>{const progress=settings.progress[id];if(!progress)return;progress.bossDefeatedByMine={};progress.bossBestByMine={};progress.bossFastestByMine={};progress.bossRewardClaimsByMine={};progress.reviewCheckpoints={};progress.activeBoss=null;progress.selectedMine=0;});saveSettings(settings,true);refreshAfterAdmin(settings);return true;
  }
  ...
  function adminResetPlacement(languageId){
    if(!multilingualAdminAllowed()||!LANGUAGES[languageId])return false;const settings=currentSettings();delete settings.placements[languageId];saveSettings(settings,true);refreshAfterAdmin(settings);return true;
  }
  function adminResetAllPlacements(){if(!multilingualAdminAllowed())return false;const result=resetSettingsSnapshot(currentSettings(),'placements:all');saveSettings(result.settings,true);refreshAfterAdmin(result.settings);return true;}
