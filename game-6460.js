...
function normalizeState(raw){
  ...
  if(!next.wallpaperPurposeSwap227){
    const oldPage=next.ownedWallpapers||[],oldMenu=next.ownedMineWallpapers||['classic'];
    next.ownedWallpapers=[...new Set([...oldPage,...oldMenu.map(id=>'color-'+id)])];
    next.ownedMineWallpapers=[...new Set([...oldMenu,...oldPage.filter(id=>id!=='midnight').map(id=>'menu-'+id)])];
    const selectedPage=next.equippedWallpaper,selectedMenu=next.equippedMineWallpaper;
    next.equippedWallpaper=selectedMenu?'color-'+selectedMenu:'midnight';
...
window.languageMinerApplyVerifiedAdminIdentity=identity=>{if(window.LANGUAGE_MINER_PREVIEW===true)return;const profile=readProfiles().find(item=>item.id===activeProfileId);isDeveloperSession=!!profile?.cloudUserId&&['owner','admin'].includes(identity?.role);if(!isDeveloperSession||identity?.role!=='owner'&&identity?.permissions?.health!==true)state.developerInfiniteHearts=false;syncDeveloperControls();};
...
function save(){
  ...
  scheduleCloudSave();
  window.dispatchEvent(new Event('lm-player-progress-saved'));
}
...
const MINE_WALLPAPERS=[
  {
    ...
    "name": "Slate Menu",
    "cost": 0,
    "desc": "Slate color background with a dark finish for readable menu controls.",
...
function renderPickaxeShop(){
  ...
  PICKAXE_SKINS.forEach(skin=>{
    ...
    const btn=card.querySelector("button");btn.dataset.patreonTier=skin.id==='standard'?'0':'1';
...
function buyPickaxe(id){
  if(!window.LanguageMinerPatreonPreview.allowed(1))return false;
...
function equipPickaxe(id){
  if(id!=="standard"&&!window.LanguageMinerPatreonPreview.allowed(1))return false;
...
function applyMineCosmetics(){
  ...
  const menu=document.querySelector('#gameMenuOverlay .game-menu-panel');
  if(menu){menu.dataset.menuWallpaper=wallpaper.id;menu.style.setProperty('--menu-wallpaper',wallpaper.preview.replace(/ center\/cover no-repeat$/, ''));}
}
...
async function loadCloudProfile(session,requestedName=""){
  ...
  const verifiedIdentity=await window.languageMinerCloudAuth?.adminIdentity?.(session),verifiedCloudAdmin=['owner','admin'].includes(verifiedIdentity?.role),verifiedOwner=verifiedIdentity?.role==='owner';
  const profiles=readProfiles(),name=cloudDisplayName(user,requestedName);
  const selectedLegacy=verifiedOwner?profiles.find(isLegacyOwnerProfile):null;
  ...
  profile.name=verifiedOwner?DEVELOPER_NAME:(profile.name||name);profile.email=user.email||profile.email||"";profile.lastPlayed=Date.now();
...
function v3SetMastery(id,delta){if(delta&&!silentTestingActive()){state.lessonPracticeStats=state.lessonPracticeStats||{};const stat=state.lessonPracticeStats[id]||{attempts:0,correct:0};stat.attempts++;stat.correct+=delta>0?1:0;state.lessonPracticeStats[id]=stat;}state.n5AcademyMastery[id]=Math.max(0,Math.min(100,academyItemMastery(id)+delta));save();renderAcademySummary();}
...
function applyWallpaper(){const supporter=(window.japaneseMinerSupporterTier?.()||0)>=1;document.body.dataset.wallpaper=supporter&&WALLPAPERS.some(w=>w.id===state.equippedWallpaper)?state.equippedWallpaper:'midnight';document.body.dataset.theme=supporter?(state.colorTheme||'midnight'):'midnight';const selected=WALLPAPERS.find(w=>w.id===document.body.dataset.wallpaper);document.body.style.setProperty('--equipped-scene',selected?.preview||'url(adventure-world.png)');}
...
document.addEventListener('click',event=>{
  ...
  if(cosmetic){
    ...
    if(key!=='skin'&&!window.LanguageMinerPatreonPreview.allowed(1,cosmetic)){event.preventDefault();event.stopImmediatePropagation();return;}
  ...
  if(theme){if(!window.LanguageMinerPatreonPreview.allowed(1,theme))return;state.colorTheme=theme.dataset.colorTheme;document.body.dataset.theme=state.colorTheme;theme.closest('.theme-choice-grid')?.querySelectorAll('button').forEach(x=>x.classList.toggle('selected',x===theme));save();render();}
},true);
...
(function(){
  ...
  function ensureV38(){
    ...
    else state.notebookView=['queue','review','stickies','learn'].includes(state.notebookView)?state.notebookView:'queue';
  ...
  function openFeatureCenter(tab='quests'){featureShell();featureTab=tab==='mistakes'?'notebook':tab;document.getElementById('featureCenterOverlay').classList.add('open');document.getElementById('featureCenterOverlay').setAttribute('aria-hidden','false');syncPageScrollLock();renderFeatureCenter(featureTab);}
  window.openJapaneseMinerNotebook=()=>openFeatureCenter('notebook');
  window.openJapaneseMinerQuests=()=>openFeatureCenter('quests');
  function closeFeatureCenter(){document.getElementById('featureCenterOverlay')?.classList.remove('open');document.getElementById('featureCenterOverlay')?.setAttribute('aria-hidden','true');syncPageScrollLock();}
  ...
  function characterMarkup(size='large',override={},presentation={}){
    ...
    if(Object.hasOwn(override,'accessory'))c.accessories=override.accessory==='none'?[]:[override.accessory];
  ...
  // Read-only presentation data shared with the pickaxe-pose wardrobe.
  window.getJapaneseMinerPoseOutfit=()=>{
    const tier=window.japaneseMinerSupporterTier?.()||0,c=state.character||{};
    const fashion=window.getJapaneseMinerEffectiveFashion?.(tier)||{jacket:'none',gloves:'none',shoes:'boots',holidaySpecial:'none'};
    return {profile:activeProfileId||'',skin:c.skin||'warm',hairStyle:tier>=1?(c.hairStyle||'short'):'spiky',hairColor:tier>=1?(c.hairColor||'brown'):'brown',accessories:tier>=1?selectedAccessories(c):[],pickaxe:state.equippedPickaxeSkin||'standard',shirt:tier>=1?(c.shirt||'miner'):'miner',pants:tier>=1?(c.pants||'denim'):'denim',...fashion};
  };
  ...
  window.renderJapaneseMinerCharacterShop=function(box){
    ...
    box.querySelectorAll('[data-character-key]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.characterKey,value=b.dataset.characterValue;if(key!=='skin'&&!window.LanguageMinerPatreonPreview.allowed(1,b))return;if(!cosmeticOwned(key,value))return;if(state.v5)state.v5.holidaySpecial='none';if(key==='accessory')setSelectedAccessories(value==='none'?[]:[value]);else state.character[key]=value;save();render();renderShop();}));window.bindJapaneseMinerAvatarApparel?.(box,()=>window.renderJapaneseMinerCharacterShop(box));
  };
  ...
  function renderFeatureCenter(tab){
    if(tab==='statistics'){closeFeatureCenter();window.openLanguageMinerMyProgress?.();return;}
    ...
    const identity=document.getElementById('featurePlayerIdentity');identity.hidden=tab!=='profile';document.querySelector('#featureCenterOverlay .feature-tabs').hidden=tab==='profile'||tab==='notebook';identity.querySelector('[data-player-name]').textContent=document.getElementById('activePlayerName')?.textContent||'Miner';identity.querySelector('[data-player-title]').textContent=state.selectedTitle||'Language Learner';identity.querySelector('[data-player-balance]').textContent='🪙 '+totalStoneValue().toLocaleString()+' Nuggets';
    ...
    content.querySelectorAll('[data-character-key]').forEach(b=>b.addEventListener('click',()=>{const key=b.dataset.characterKey,value=b.dataset.characterValue;if(key!=='skin'&&!window.LanguageMinerPatreonPreview.allowed(1,b))return;if(state.v5)state.v5.holidaySpecial='none';if(key==='accessory')setSelectedAccessories(value==='none'?[]:[value]);else state.character[key]=value;save();renderFeatureCenter('profile');render();}));
    ...
    if(tab==='notebook')window.LanguageMinerPictures?.bindNotebook?.(content);
    ...
    content.querySelectorAll('[data-title]').forEach(b=>b.addEventListener('click',()=>{if(b.dataset.titleAlpha==='true'&&!window.LanguageMinerAdminPrivileges?.isAlphaTester?.())return;const free=b.dataset.titleFree==='true';if(!free&&(window.japaneseMinerSupporterTier?.()||0)<1)return;state.selectedTitle=b.dataset.title;save();renderFeatureCenter('achievements');render();setMessage(`${state.selectedTitle} title equipped.`,'correct');}));
    content.querySelectorAll('[data-notebook-view]').forEach(b=>b.addEventListener('click',()=>{state.notebookView=['queue','review','stickies','learn'].includes(b.dataset.notebookView)?b.dataset.notebookView:'queue';save();renderFeatureCenter('notebook');}));
  ...
  function addMenuItems(){const grid=document.querySelector('.game-menu-grid');if(!grid)return;grid.querySelectorAll('[data-feature-open="achievements"],[data-feature-open="notebook"]').forEach(button=>button.remove());const items=[['profile','🧍','Character','Customize hair, skin, and clothing','gear'],['quests','🎯','Goals','Daily, weekly, and expedition objectives','adventure'],['account','☁️','Account & Support','Backup, privacy, and supporter membership','player']];items.forEach(([tab,icon,name,desc,category])=>{if(grid.querySelector(`[data-feature-open="${tab}"],[data-menu-action="${tab}"]`))return;const b=document.createElement('button');b.type='button';b.dataset.featureOpen=tab;b.dataset.menuCategoryName=category;b.innerHTML=`<span>${icon}</span><strong>${name}</strong><small>${desc}</small>`;b.addEventListener('click',()=>{closeGameMenu();openFeatureCenter(tab);});grid.appendChild(b);});}
  window.refreshJapaneseMinerFeatureMenu=addMenuItems;
  window.openLanguageMinerStats=()=>window.openLanguageMinerMyProgress?.();
  window.LanguageMinerPersonalStats=Object.freeze({render:renderStatistics,calendar:()=>{window.LanguageMinerParentTeacher?.close();openStudyCalendar();},placement:language=>{window.LanguageMinerParentTeacher?.close();if(language)document.getElementById('lmChangeLanguageBtn')?.click();else openPlacementOnboarding(false);}});
...
function setVocabularyWordMastery(word,delta){
  if(!word)return;
  v3SetMastery(word.masteryId,Number(delta||0));
}
...
(()=>{
  ...
  function summarizeLessons(lessons){
    ...
    const completedItems=lessons.filter(lesson=>lesson.completed);const completedAverageMastery=completedItems.length?Math.round(completedItems.reduce((sum,lesson)=>sum+lesson.mastery,0)/completedItems.length):null;
    return {completedAverageMastery,items:lessons,counts:{total,completed,inProgress,available,locked},completionPercent:total?Math.round(completed/total*100):0,averageMastery};
  }
  function japaneseLessonSummary(source){
    ...
    for(let stage=0;stage<Math.min(2,stages.length);stage++){
      ...
      families.forEach((family,index)=>{const values=family.entries.map(([character])=>masteryFromStat(source.kanaStats?.[character],25)),mastery=values.length?Math.round(values.reduce((sum,value)=>sum+value,0)/values.length):0,required=KANA_FAMILY_UNLOCK_MASTERY,unlocked=stageOpen(stage)&&(index===0||passedStage||previousComplete),masteredItems=values.filter(value=>value>=required).length,completed=mastery>=required;lessons.push({practiceKeys:family.entries.map(([character])=>"kana:"+character),id:`ja:${stage}:alphabet:${index}`,course:'Japanese',level:stages[stage]?.label||`Level ${stage+1}`,stage,section:'Alphabet',lesson:index+1,name:family.name,mastery,required,masteredItems,totalItems:values.length,completed,unlocked});previousComplete=completed;});
    }
    for(let stage=2;stage<stages.length;stage++){
      JLPT_SECTION_SPECS.forEach(spec=>{const levels=jlptSectionLevels(stage,spec.id);levels.forEach((items,index)=>{const values=items.map(item=>itemMastery(item.masteryId)),mastery=values.length?Math.round(values.reduce((sum,value)=>sum+value,0)/values.length):0,previous=index?lessons.find(lesson=>lesson.id===`ja:${stage}:${spec.id}:${index-1}`):null,checkpoint=index>0&&index%2===0?source.jlptReviewCheckpoints?.[`${stage}:${spec.id}:${index}`]:null,unlocked=stageOpen(stage)&&(index===0||(previous?.completed===true&&(index%2!==0||checkpoint?.passed===true))),masteredItems=values.filter(value=>value>=JLPT_VOCABULARY_UNLOCK_MASTERY).length;lessons.push({practiceKeys:items.map(item=>item.masteryId),id:`ja:${stage}:${spec.id}:${index}`,course:'Japanese',level:stages[stage]?.label||`Level ${stage+1}`,stage,section:spec.name,lesson:index+1,name:`${spec.name} Lesson ${index+1}`,mastery,required:JLPT_VOCABULARY_UNLOCK_MASTERY,masteredItems,totalItems:values.length,completed:mastery>=JLPT_VOCABULARY_UNLOCK_MASTERY,unlocked});});});
    }
  ...
  function multilingualLessonSummary(settings,learning,progress){
    ...
    if(travel){const values=(data.sentences||[]).filter(item=>[1,2,3,4,7,11].includes(Number(item.topic)));for(let offset=0;offset<values.length;offset+=10){const items=values.slice(offset,offset+10),scores=items.map(item=>mastery('travel',item)),value=scores.length?Math.round(scores.reduce((sum,score)=>sum+score,0)/scores.length):0;lessons.push({practiceKeys:items.map(item=>`travel:${item.id??item.symbol??""}`),id:`${learning}:0:travel:${offset/10}`,course:LANGUAGE_NAMES[learning],level:'Travel course',stage:0,section:'Travel phrases',lesson:offset/10+1,name:`Travel Lesson ${offset/10+1}`,mastery:value,required:75,masteredItems:scores.filter(score=>score>=75).length,totalItems:items.length,completed:value>=75,unlocked:offset===0||lessons[lessons.length-1]?.completed===true});}return summarizeLessons(lessons);}
    const alphabetEntries=Object.entries(progress.courseMastery||{}).filter(([key])=>key.startsWith('alphabet:')),alphabetScores=alphabetEntries.map(([,value])=>Math.max(0,Math.min(100,safeNumber(value))));if(alphabetScores.length){const value=Math.round(alphabetScores.reduce((sum,score)=>sum+score,0)/alphabetScores.length);lessons.push({practiceKeys:alphabetEntries.map(([key])=>key),id:`${learning}:0:alphabet:0`,course:LANGUAGE_NAMES[learning],level:'Level 1',stage:0,section:'Alphabet',lesson:1,name:'Alphabet Lesson',mastery:value,required:75,masteredItems:alphabetScores.filter(score=>score>=75).length,totalItems:alphabetScores.length,completed:value>=75,unlocked:true});}
    for(let mine=1;mine<=6;mine++){
      const mineOpen=defeated[mine-1]===true;[['vocabulary',25],['grammar',10],['sentences',10]].forEach(([section,size])=>{const stageItems=splitReportItems(data[section]||[],6)[mine-1]||[],sectionLessons=[];for(let offset=0;offset<stageItems.length;offset+=size)sectionLessons.push(stageItems.slice(offset,offset+size));sectionLessons.forEach((items,index)=>{const scores=items.map(item=>mastery(section,item)),value=scores.length?Math.round(scores.reduce((sum,score)=>sum+score,0)/scores.length):0,previous=index?lessons.find(lesson=>lesson.id===`${learning}:${mine}:${section}:${index-1}`):null,checkpoint=index>0&&index%2===0?progress.reviewCheckpoints?.[`${mine}:${section}:${index}`]:null,unlocked=mineOpen&&(index===0||(previous?.completed===true&&(index%2!==0||checkpoint?.passed===true)));lessons.push({practiceKeys:items.map(item=>`${section}:${item.id??item.symbol??""}`),id:`${learning}:${mine}:${section}:${index}`,course:LANGUAGE_NAMES[learning],level:`Level ${mine+1}`,stage:mine,section:SECTION_NAMES[section],lesson:index+1,name:`${SECTION_NAMES[section]} Lesson ${index+1}`,mastery:value,required:75,masteredItems:scores.filter(score=>score>=75).length,totalItems:items.length,completed:value>=75,unlocked});});});
    }
    return summarizeLessons(lessons);
  }
  ...
  function assessmentSummary(source,settings){
    ...
    history.sort((a,b)=>(b.completedAt||0)-(a.completedAt||0));const scored=history.filter(record=>record.group!=='placement'&&record.difficulty!=='placement'&&!/placement/i.test(record.type||'')&&record.course===(LANGUAGE_NAMES[learning]||'Japanese')&&(!record.total||record.answered>=record.total)&&!['abandoned','cancelled','canceled'].includes(record.finishReason)&&Number.isFinite(Number(record.score))),passedCount=history.filter(record=>record.result==='Passed'||record.result==='Completed'||record.result==='Perfect').length,easyCount=history.filter(record=>record.difficulty==='easy').length,hardCount=history.filter(record=>record.difficulty==='hard').length;
    return {gradedCount:scored.length,history:history.slice(0,LEARNING_REPORT_MAX_ATTEMPTS),fastest,attemptCount:history.reduce((sum,record)=>sum+Math.max(1,safeNumber(record.attempts)),0),passedCount,averageScore:scored.length?Math.round(scored.reduce((sum,record)=>sum+safeNumber(record.score),0)/scored.length):0,easyCount,hardCount,modeUnknownCount:history.filter(record=>record.difficulty==='not-recorded').length};
  }
  function gradingSummary(questions,course,assessments,activity,reviews){
    const components=[],assessmentAvailable=assessments.gradedCount>0,lessonMastery=safeNumber(course.lessons?.completedAverageMastery),lessonCompletion=safeNumber(course.lessons?.completionPercent);if(assessmentAvailable)components.push({id:'assessments',label:'Quiz & test average',score:assessments.averageScore,weight:50});if(course.lessons?.counts?.completed)components.push({id:'lessons',label:'Completed lesson mastery',score:lessonMastery,weight:30});if(questions.answered)components.push({id:'practice',label:'Completed-lesson practice accuracy',score:questions.accuracy,weight:20});const weight=components.reduce((sum,item)=>sum+item.weight,0),available=weight>0,score=available?(components.every(item=>item.score===100)?100:Math.min(99.9,Math.round(components.reduce((sum,item)=>sum+item.score*item.weight,0)/weight*10)/10)):0,letter=!available?'—':score>=90?'A':score>=80?'B':score>=70?'C':score>=60?'D':'F',flags=[];if(!activity.lastStudyDate)flags.push({level:'attention',text:'No study activity has been recorded yet.'});else if(activity.activeDaysLast7===0)flags.push({level:'attention',text:'No active study day was recorded in the last 7 days.'});else if(activity.streakAtRisk)flags.push({level:'watch',text:'The current streak is at risk unless the learner studies today.'});if(questions.answered>=10&&questions.accuracy<75)flags.push({level:'attention',text:`Practice accuracy is ${questions.accuracy}%, below the 75% progression target.`});if(assessmentAvailable&&assessments.averageScore<75)flags.push({level:'attention',text:`Assessment average is ${assessments.averageScore}%, below the 75% quiz target.`});if(reviews.dueCount>0)flags.push({level:'watch',text:`${reviews.dueCount} Smart Review item${reviews.dueCount===1?' is':'s are'} due.`});if(course.lessons?.counts?.inProgress)flags.push({level:'info',text:`${course.lessons.counts.inProgress} lesson${course.lessons.counts.inProgress===1?' is':'s are'} currently in progress.`});if(!flags.length)flags.push({level:'good',text:'No immediate learning risks are visible in the recorded data.'});components.forEach(item=>item.weight=Math.round(item.weight/weight*100));return {available,score,letter,components,lessonCompletion,flags,formula:'50% completed quizzes/tests · 30% completed lesson mastery · 20% practice accuracy for completed lessons. Placement tests and unfinished work are excluded. Missing categories are ungraded; remaining weights are rebalanced.'};
  }
  function completedLessonPractice(source,settings,course){
    const keys=new Set(course.lessons.items.filter(lesson=>lesson.completed).flatMap(lesson=>lesson.practiceKeys||[]));let answered=0,correct=0;
    for(const key of keys){const stat=course.learning==='ja'?(key.startsWith('kana:')?source.kanaStats?.[key.slice(5)]:source.lessonPracticeStats?.[key]):settings.progress?.[course.learning]?.itemPracticeStats?.[key];
      const attempts=Math.max(0,safeNumber(stat?.attempts??stat?.a));answered+=attempts;correct+=Math.min(attempts,Math.max(0,safeNumber(stat?.correct??stat?.c)));
    }
    return {answered,correct,accuracy:answered?(correct===answered?100:Math.min(99.9,Math.round(correct/answered*1000)/10)):0};
  }
  ...
  function snapshot(profileId){
    ...
    return summarize(profile,profile.id===activeProfileId?state:profileState(profile.id),profile.id===activeProfileId?(window.LanguageMinerCourseCloud?.exportCurrent?.()||courseSettings(profile)):courseSettings(profile));
  }
